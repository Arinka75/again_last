...
public class PreProcessController {
    ...
    void initialize(){
        ...
        saveButton.setOnAction(actionEvent -> {
            ...
            if (!firstNodeBlock.isSelected() && !lastNodeBlock.isSelected()) {
                // Если оба чекбокса не выбраны, выводим предупреждение и выходим из метода
                Alert alert = new Alert(Alert.AlertType.WARNING);
                alert.setTitle("Предупреждение");
                alert.setHeaderText(null);
                alert.setContentText("Пожалуйста, выберите хотя бы один блок узла (первый или последний).");
                alert.showAndWait();
